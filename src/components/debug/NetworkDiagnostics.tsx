...
export const NetworkDiagnostics: React.FC = () => {
  ...
  const runDiagnostics = async () => {
    ...
    if (supabaseUrl && supabaseKey) {
      ...
      logger.error('Environment test: Missing variables', {
        hasUrl: !!supabaseUrl,
        hasKey: !!supabaseKey,
      });
    }
    ...
    if (isSimulator) {
      ...
      try {
        ...
        updateTest(
          'Simulator Mode',
          'success',
          'Auth methods available (network tests skipped in simulator)',
          {
            ...authMethods,
      ...
      updateTest(
        'Recommendations',
        'success',
        'Use Expo Go app or physical device for full testing',
        {
          alternatives: [
      ...
      try {
        ...
        if (error) {
          ...
          updateTest('Supabase Auth', 'success', 'Supabase auth working', {
            ...
            sessionValid: data.session?.expires_at
  ...
  const getStatusColor = (status: NetworkTest['status']) => {
    switch (status) {
      case 'success':
        return styles.statusSuccess.color;
      case 'failed':
        return styles.statusFailed.color;
      case 'pending':
        return styles.statusPending.color;
      default:
      ...
      {tests.map((test, index) => (
        <View key={index} style={styles.test}>
          ...
          {test.details && __DEV__ && (
            <Text style={styles.debugDetails}>{JSON.stringify(test.details, null, 2)}</Text>
          )}
        </View>
      ))}

      <Button mode="contained" onPress={runDiagnostics} disabled={isRunning} style={styles.button}>
