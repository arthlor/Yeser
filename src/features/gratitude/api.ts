...
export const addStatement = async (
  ...
  try {
    ...
    const { error } = await client.rpc('add_gratitude_statement', rpcParams);
...
export const deleteStatement = async (entryDate: string, statementIndex: number): Promise<void> => {
  try {
    ...
    const { error } = await client.rpc('delete_gratitude_statement', {
      p_entry_date: entryDate,
      p_statement_index: statementIndex,
    });
...
export const deleteEntireEntry = async (entryDate: string): Promise<void> => {
  try {
    ...
    const { error } = await client.rpc('delete_gratitude_entry_by_date', {
      p_entry_date: entryDate,
    });
...
export const getEntryDatesForMonth = async (year: number, month: number): Promise<string[]> => {
  try {
    const { client, session } = await getAuthedClient();
    const { user } = session;
    const { data, error } = await client.rpc('get_entry_dates_for_month', {
      p_user_id: user.id,
      p_year: year,
      p_month: month,
    });
...
export const getTotalGratitudeEntriesCount = async (): Promise<number> => {
  try {
    const { client } = await getAuthedClient();
    const { data, error } = await client.rpc('get_user_gratitude_entries_count');
